...
final case class RuntimeCatalog(
  ...
  def resolve(selector: String): RuntimeCatalogVersion = {
    val version =
      selector match {
        case "recommended" =>
          recommended.getOrElse(throw CozyException("runtime catalog does not define recommended"))
        case "latest" | "latest-stable" | "latest.release" =>
          latestStable.getOrElse(throw CozyException("runtime catalog does not define latestStable"))
        case "latest-snapshot" =>
          latestSnapshot.getOrElse(throw CozyException("runtime catalog does not define latestSnapshot"))
        case "newest" =>
          return newest
        case x =>
          x
    ...
    versions.find(_.version == version)
      .getOrElse(throw CozyException(s"runtime version is not listed in catalog: $version"))
      .validated
  }
...
final case class RuntimeCatalogVersion(
  ...
  def validated: RuntimeCatalogVersion =
    status match {
      case Some("disabled") =>
        throw CozyException(s"runtime version is disabled: $version")
      case _ =>
        this
    }
  ...
  def moduleCoordinate: String =
    module.getOrElse(s"org.simplemodeling:cozy_2.12:$version")
